...
<body>
    ...
    <div style="vertical-align:middle">
        搜索&nbsp;<input type="text" id="searchInfo" maxlength="10" ><img src="${serviceaddress}/background/search-1.png" 
    ...
    <div id="mainSwiper" class="swiper-container" >
        ...
        <div class="swiper-wrapper" >
            <div class="swiper-slide" id="commenDiv"><!-- 需要加载完成后动态设定高度 -->
                <div style="color:white;font-size:17px">电影推荐</div>
                <table  style="width:335px; height=:164px;">
                	<tr>
                		<td >
				        	<div id="movieTuijian" class="swiper-container" style="width:335px; height=:164px;">
				        		<div id="movieTuijianTBody" class="swiper-wrapper" style="position:relative">
				        		</div>
				        	</div>
				        </td>
                	</tr>
                </table>
                <div id="message" style="color:white;font-size: 17px">电影资源</div>
    ...
    $(function(){//页面元素加载完毕执行
        ...
        var swiper = new Swiper('#movieTuijian', {
    ...
    function displayData(pageNo){
		...
		//查询推荐电影
		$.ajax({
			url:"${pageContext.request.contextPath}/playMovie/getTuijian.do",
			type:"post",
			cache:false,
			async: false,
			data:{
			...
			success:function(jsonObject){
				$("#movieTuijianTBody").empty();
				var htmlString = "";
				$.each(jsonObject.dataList,function(index,dom){//onclick="jumpPlay(reid)"
				/* 电影推荐模板
					        <div class="swiper-slide" >
					              <img src="" width="315px" height="164px"/>
		                			<div style="color:white;font-size: 20px;position: absolute;top: 140px;left: 30px;z-index: 2;"></div>
					        </div>
					         <div class="swiper-slide" >
					                <img src="" width="315px" height="164px"/>
		                			<div style="color:white;font-size: 20px;position: absolute;top: 140px;left: 30px;z-index: 2;"></div>
					...
					*/
					...
					htmlString += "<img src='${serviceaddress}/"+dom.picpath+"' onclick='jumpPlay(\""+ dom.reid +"\")' width='335px' height='164px' style='margin-left:-10px;' />";
					htmlString += "<div style='color:white;font-size: 20px;position: absolute;top: 140px;left: 30px;z-index: 2;'>"+dom.showname+"</div>";
					htmlString += "</div>";
					
				});//each结束
				$("#movieTuijianTBody").append(htmlString);
			}
		});
					...
					$.each(jsonObject.dataList,function(index,dom){
						...
						*/
						...
						if(index%2==0){//说明是首列，根据模板格式拼接。这个index从0开始
							...
							htmlString += "<img src='${serviceaddress}/"+dom.picpath+"'style='width: 160px;height: 100px;' onclick='jumpPlay(\""+ dom.reid +"\")'/>"; 
		...
		addCount = 0; //重新归零
		$("#commenDiv").height(newHeight);
		//重新创建swiper，因为过去的会失效，同时保证计算的页高度正确结合重新设定高度方法实现
		 var swiper = new Swiper('#mainSwiper', {
    		scrollbar: '.swiper-scrollbar',
            direction: 'vertical',
            slidesPerView: 'auto',
    ...
    function jumpPlay(reid){//发送id重新查找
    	...
    	window.location.href="${serviceaddress}:8080/ms/playMovie/play.do"+"?reid="+reid;
    }
    </script>
</body>
</html>